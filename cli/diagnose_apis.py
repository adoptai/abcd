...
from wdl_common.models import IssueType, Severity


def fetch_all_apis(bearer_token: str, api_endpoint: str | None = None) -> list[dict]:
    ...
    while True:
        ...
        if isinstance(json_response, list):
            ...
            page_apis = (
                json_response.get("apis")
                or json_response.get("data")
                or json_response.get("items")
                or []
            )
...
""",
    ...
    parser.add_argument(
        "--filter",
        choices=["trailing_slash", "workflow_arguments", "query_params"],
        help="Filter by issue type",
    )
    parser.add_argument(
        "--min-score", type=float, default=0.7, help="Minimum match score (default: 0.7)"
    )
