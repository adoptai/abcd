...
def parse_log_entry(entry: dict[str, Any]) -> NetworkLogEntry | None:
    """
    ...
    log_entry = NetworkLogEntry(
        ...
        api_endpoint_url=str(entry.get("api_endpoint_url", entry.get("apiEndpointUrl", ""))),
...
def load_network_logs(json_file_path: str) -> list[NetworkLogEntry]:
    ...
    try:
        ...
        raise ValueError(f"Invalid JSON in network logs file: {e}") from e
...
def filter_logs_by_method(logs: list[NetworkLogEntry], methods: list[str]) -> list[NetworkLogEntry]:
...
def filter_logs_by_path_pattern(logs: list[NetworkLogEntry], pattern: str) -> list[NetworkLogEntry]:
    """Filter logs by path pattern (substring match)."""
    pattern_lower = pattern.lower()
    return [log for log in logs if pattern_lower in log.parsed_path.lower()]


def filter_logs_by_host(logs: list[NetworkLogEntry], host: str) -> list[NetworkLogEntry]:
