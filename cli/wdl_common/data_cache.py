...
class DataCache:
    ...
    def __init__(self, cache_dir: str | Path | None = None):
        ...
        if cache_dir is not None:
            ...
            try:
                from cli.wdl_common.context import ensure_env, get_env_cache_path
                ...
                print(
                    f"⚠️  Using global cache (no active environment): {self.cache_dir}",
                    file=sys.stderr,
                )
        ...
        self._apis_cache: list[dict] | None = None
        self._logs_cache: list[NetworkLogEntry] | None = None
    ...
    def _get_cache_metadata(self, cache_file: Path) -> dict[str, Any] | None:
    ...
    def get_cache_age(self, cache_type: str) -> datetime | None:
        """
    ...
    def is_cache_stale(self, cache_type: str, max_age_hours: int = 24) -> bool:
        """
    ...
    def load_tools_cache(self) -> list[dict] | None:
    ...
    def load_apis_cache(self) -> list[dict] | None:
        ...
        try:
            with open(self.apis_cache_file) as f:
                data = json.load(f)
    ...
    def load_logs_cache(self) -> list[NetworkLogEntry] | None:
        ...
        try:
            with open(self.logs_cache_file) as f:
                data = json.load(f)
                ...
                self._logs_cache = [
                    log
                    for log in (parse_log_entry(entry) for entry in logs_data)
                    if log is not None
                ]
                return self._logs_cache
        except (OSError, json.JSONDecodeError):
    ...
    def display_cache_status(self) -> None:
        ...
        for cache_type, info in status.items():
            if info["exists"]:
                ...
                print(
                    f"  {cache_type.upper()}: {info['count']} items, cached at {age_str}{stale_indicator}"
                )
            else:
