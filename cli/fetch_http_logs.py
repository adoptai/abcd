...
from wdl_common.log_parser import get_logs_stats, save_logs_to_file
...
def fetch_network_logs_from_api(
    ...
    api_endpoint = api_endpoint or os.getenv("ADOPT_API_ENDPOINT") or "https://connect.adopt.ai"
            ...
            if response.status_code in RETRY_STATUS_CODES:
                if retry_count >= max_retries:
                    if skip_failed_pages:
                        print(
                            f"⏭️  Skipping page {params.get('page')} after {max_retries} failed attempts"
                        )
                        return None
                ...
                print(
                    f"⏳ Rate limited. Waiting {wait_time:.1f}s (retry {retry_count + 1}/{max_retries})..."
                )
                sleep(wait_time)
...
""",
    ...
    if args.inspect:
        ...
        if args.search:
            ...
            filtered_logs = [
                log for log in filtered_logs if args.search.lower() in log.api_endpoint_url.lower()
            ]
