...
def show_status(workflow_id: str, sync_remote: bool = True) -> bool:
    ...
    if working.number:
        ...
        print(
            f"│ Version:    {working.number} ({status_text}) {status_icon}"
            + " " * (60 - len(str(working.number)) - len(status_text))
            + "│"
        )
        ...
        if wdl_path.exists():
            try:
                ...
                print(
                    f"│ Operations: {op_count} operations in widdle.json"
                    + " " * (50 - len(str(op_count)))
                    + "│"
                )
            except Exception:
        ...
        if working.description:
            desc = (
                working.description[:50] + "..."
                if len(working.description) > 50
    ...
    if action_id and sync_remote:
        ...
        if success and rules:
            ...
            if targeting_rules:
                print(
                    f"│ Targeting Rules:  {len(targeting_rules)} rules configured" + " " * 43 + "│"
                )
            else:
    ...
    if remote.latest_draft or remote.latest_published or remote.current:
        if remote.latest_draft:
            ...
            print(
                f"│ Latest Draft:     {remote.latest_draft}{match_text}"
                + " " * (59 - len(str(remote.latest_draft)) - len(match_text))
                + "│"
            )
        else:
        ...
        if remote.latest_published:
            print(
                f"│ Latest Published: {remote.latest_published}"
                + " " * (59 - len(str(remote.latest_published)))
                + "│"
            )
        else:
        ...
        if remote.current:
            print(
                f"│ Current (Live):   {remote.current}"
                + " " * (59 - len(str(remote.current)))
                + "│"
            )
    else:
        if action_id:
    ...
    if metadata.versions:
        ...
        sorted_versions = sorted(metadata.versions.items(), key=lambda x: int(x[0]), reverse=True)[
            :5
        ]  # Show last 5
        ...
        for v_num, v_info in sorted_versions:
            ...
            desc = (
                v_info.description[:35] + "..."
                if v_info.description and len(v_info.description) > 35
                else (v_info.description or "")
            )
    ...
    if not action_id:
        ...
        print("│" + " " * 78 + "│")
        print(
            f"│ 📦 Publish version {working.number}:" + " " * (56 - len(str(working.number))) + "│"
        )
