...
def fetch_tool_details(
    ...
    actions_endpoint = actions_endpoint or os.getenv(
...
def fetch_all_tools(bearer_token: str, api_endpoint: str | None = None) -> list[dict]:
    ...
    try:
        ...
        if args.tool_id:
            ...
            if args.search:
                ...
                # Filter by API ID
                tools_to_inspect = [t for t in all_tools if args.api_id in (t.get("api_ids") or [])]
        ...
        for i, tool in enumerate(tools_to_inspect):
            ...
            if not args.verbose and len(tools_to_inspect) > 1:
                print(
                    f"   [{i + 1}/{len(tools_to_inspect)}] Inspecting {tool.get('title', tool_id)[:40]}...",
                    end=" ",
                )
