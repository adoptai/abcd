...
def fetch_api_from_remote(bearer_token: str, api_id: str) -> Optional[Dict]:
    """
...
def fix_single_api(
    ...
    new_path_override: Optional[str] = None,
) -> Tuple[bool, str]:
    """
    ...
    if not api:
        print(f"   API not in cache, fetching from remote...")
        api = fetch_api_from_remote(bearer_token, api_id)
    
    if not api:
        return False, f"API not found: {api_id}. Run 'python cli/diagnose_and_fix.py --scan' to populate cache, or use --path to specify the new path directly."
...
""",
    ...
    parser.add_argument("--trailing-slash", choices=['add', 'remove'],
                       help="Add or remove trailing slash")
