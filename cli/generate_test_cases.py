...
from wdl_common.auth import get_bearer_token_for_env
...
def generate_basic_test_prompts(
    ...
    for input_name, input_def in required_inputs.items():
        ...
        input_def.get("definition", "") if isinstance(input_def, dict) else str(input_def)
...
def generate_llm_test_prompts(
    ...
    Returns:
        List of test case objects
    """
    AdoptAPIClient(bearer_token)

    f"""Generate 5 realistic test prompts for the following tool:
...
""",
    ...
    try:
        ...
        if args.use_llm:
            bearer_token = get_bearer_token_for_env(env_name)
