...
def fetch_all_apis(bearer_token: str) -> list[dict]:
    ...
    while True:
        response = requests.get(
            url, headers=headers, params={"page": page, "page_size": 50}, timeout=30
        )
...
def fetch_tool_details(bearer_token: str, tool_id: str) -> dict | None:
...
def run_comprehensive_scan(
    ...
    for _i, tool in enumerate(tools[:100]):  # Limit to first 100 for performance
        tool_id = tool.get("id")
        if not tool_id:
            continue
...
def _create_api_issue(api: dict, match_result) -> DiagnosticIssue:
    ...
    return DiagnosticIssue(
        ...
        details={
            ...
            "matched_log_url": match_result.best_match.api_endpoint_url
...
def generate_markdown_report(report: dict[str, Any]) -> str:
    ...
    for issue_type, count in sorted(
        report["summary"]["issues_by_type"].items(), key=lambda x: -x[1]
    ):
                ...
                if new_path:
                    ...
                    if success:
                        rollback_manager.add_api_change(
                            api_id, title, change.get("old_value", ""), new_path
                        )
...
""",
    ...
    parser.add_argument(
        "--format", choices=["llm", "markdown"], default="llm", help="Output format (default: llm)"
    )
