...
def save_single_draft(
    ...
    if dry_run:
        ...
        log(f"\n🔍 [DRY-RUN] Would save draft for: {workflow_id}")
        log(f"   Workspace   : {workspace}")
        log(f"   WDL ops     : {len(wdl)}")
        log(f"   Action ID   : {action_id or '(would create new action)'}")
        log(f"   Description : {description or 'WDL workflow update'}")
        log("\n   ✅ Validation passed — run without --dry-run to save to remote")
        return DraftResult(
            workflow_id=workflow_id,
            success=True,
            message="[DRY-RUN] Would save draft",
            action_id=action_id,
        )
    ...
    log(f"\n🔑 Action ID: {action_id}")
    _vprint(f"Action ID confirmed: {action_id}")

    if not action_id:
        return DraftResult(
            workflow_id=workflow_id,
            success=False,
            message="Could not resolve action ID",
            error="No action_id available after all recovery attempts",
        )
    ...
    _vprint(f"API call: publish_wdl({action_id}, draft_id={draft_id!r})")
...
def save_parallel_drafts(
    ...
    print(
        f"\n🚀{dry_label} Saving {len(workflow_ids)} drafts in parallel (max {max_workers} workers)..."
    )
    print("=" * 80)
...
def save_agent_drafts(
    ...
    if (agent_path / "widdle.json").exists():
        print(f"\n📦 {'[DRY-RUN] Would save' if dry_run else 'Saving'} agent WDL...")
        client = get_api_client_for_env()
        agent_result = save_single_draft(agent_id, manager, client, description, True, dry_run)
...
Examples:
  ...
  python save_wdl_draft.py --workflow-id my-workflow
    ...
    parser.add_argument(
        "actions", nargs="*", help="Action/workflow ID(s) to save (supports multiple)"
    )

    # Agent mode
    parser.add_argument("--agent", "-a", help="Save agent and all its subactions")
    parser.add_argument(
        "--force", "-f", action="store_true", help="Save all subactions (not just changed ones)"
    )
    ...
    # Legacy flags
    parser.add_argument("--action-id", help="(legacy) Action ID")
    ...
    # Common options
    parser.add_argument("--description", "-d", help="Description of changes for this version")
    parser.add_argument("--env", "-e", help="Environment to use")
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Simulate the save without making any remote API calls or local file writes",
    )
    parser.add_argument(
        "--verbose",
        "-v",
    ...
    if args.agent:
        ...
        # Legacy mode
        client = get_api_client_for_env()
        wid = args.workflow_id or args.action_id
        result = save_single_draft(
            wid,
            manager,
            client,
